...
ModuleCollider = function(){
    this.collisionChecks = [];
};

ModuleCollider.prototype.init = function(){
    console.log("Initializing ModuleCollider, I am responsible for checking collisions");
};

ModuleCollider.prototype.CheckCollision = function(){
    // console.log("this.collisionChecks.length: ", this.collisionChecks);
    for(var i = 0; i < this.collisionChecks.length; i++){
        Game.physics.arcade.collide(this.collisionChecks[i].a, this.collisionChecks[i].b);
    }
    return true;
};

ModuleCollider.prototype.AddCollisionCheck = function(groupNameB, groupNameA){
    this.collisionChecks.push({a: Groups[groupNameA.value], b: Groups[groupNameB.value]});
    return true;
};

ModuleCollider.prototype.CheckOverlap = function(groupName, dataSprite, signal_overlap, signal_notOverlap){
    if (groupName.value in Groups) {
        var groupObj = Groups[groupName.value];
        Game.physics.arcade.overlap(dataSprite.SpriteObject.value, groupObj, this.overlapped(signal_overlap), null, this);
    }
    return true;

};

ModuleCollider.prototype.overlapped = function(signal_overlap){

    this.signal_overlap = signal_overlap;
    return function (sprite1, sprite2) {
        sprite2.kill();
        this.signal_overlap.fire();
    };
};