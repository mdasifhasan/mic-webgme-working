...
/**
 * Generated by PluginGenerator 1.7.0 from webgme on Tue Oct 04 2016 16:23:51 GMT-0500 (Central Daylight Time).
...
define([
    ...
    FamilyTransformation.prototype.main = function (callback) {
        ...
        self.loadNodeMap(nodeObject).then(function (nodes) {
        ...
        self.save('FamilyTransformation updated model.')
            .then(function () {
                self.result.setSuccess(true);
                callback(null, self.result);
            })
        ...
        FamilyTransformation.prototype.loadNodeMap = function (activeNode) {
            var self = this;

            return self.core.loadSubTree(activeNode).then(function (nodesArr) {
               var node = {}, i;
                for(i=0; i < nodesArr.length; i+=1){
                    var nodePath = self.core.getPath(nodesArr[i]);
                    nodes[nodePath] = nodesArr[i];
                }
                return nodes;
            });
        }
